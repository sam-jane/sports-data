...
```{r}
combined <- bind_rows(
  matches_2024 %>% mutate(year = 2024),
  matches_2025 %>% mutate(year = 2025)
)

ttest_all <- t.test(total_goals ~ year, data = combined)
ttest_all
```
```{r}
umd_2024 <- matches_2024 %>% 
  filter(team == "Maryland")

umd_2025 <- matches_2025 %>% 
  filter(team == "Maryland")

umd_combined <- bind_rows(
  umd_2024 %>% mutate(year = 2024),
  umd_2025 %>% mutate(year = 2025)
)

ttest_umd <- t.test(total_goals ~ year, data = umd_combined)
ttest_umd
